...
export const deleteFirebaseTodo = async (todoId: string) => {
  try {
    ...
    await firestore().collection('users').doc(user.uid).collection('todos').doc(todoId).delete();
    return true;
  } catch (error) {
...
export const fetchAllTodos = async () => {
  try {
    ...
    const snapshot = await firestore().collection('users').doc(user.uid).collection('todos').get();

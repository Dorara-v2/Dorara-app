  ...
  return (
    <ScreenContent>
      <View className="flex-row items-center p-4">
        <TouchableOpacity onPress={() => navigation.goBack()} className="mr-4 rounded-full p-2">
      ...
      <ScrollView className="flex-1 px-4">
        {/* Profile Card */}
        <View className="mb-6 overflow-hidden rounded-xl bg-white dark:bg-[#1f1f1f]">
          <View className="p-6">
            <View className="mb-4 flex-row items-center justify-between">
              ...
              />
              <TouchableOpacity className="flex-row items-center rounded-lg bg-[#f3a49d] px-4 py-2">
                <MaterialIcon name="edit" size={20} color="white" />
                <Typo className="ml-2 font-medium text-white">Edit Profile</Typo>
              </TouchableOpacity>
            </View>
            <View>
              <Typo className="mb-1 text-2xl font-bold">{user.displayName}</Typo>
        ...
        <View className="mb-6 overflow-hidden rounded-xl bg-white dark:bg-[#1f1f1f]">
          ...
          />
          <Divider />
          <SettingItem icon="folder" label="Folder" value="dorara-v2" />
          <Divider />
          <SettingItem icon="access-time" label="Last Sync" value={syncTime} />
          {!user.isGuest && (
            <>
              <Divider />
              <TouchableOpacity
                onPress={handleForceSync}
                className="flex-row items-center justify-center bg-[#f3a49d]/10 p-4">
                <MaterialIcon name="sync" size={20} color="#f3a49d" />
                <Typo className="ml-2 font-medium text-[#f3a49d]">Force Sync Now</Typo>
              </TouchableOpacity>
            </>
          )}
        </View>
...
const SettingItem = ({
  ...
  <View className="flex-row items-center justify-between p-4">
    ...
    {right || (value && <Typo>{value}</Typo>)}
  </View>
);

const Divider = () => <View className="h-[1px] bg-gray-100 dark:bg-gray-800" />;
